...
from evaluator_gate import check_visible_artifact
...
JUDGE_PAGES = tuple(
    VISIBLE_ROOT / "pages" / f"{index:02d}-{slug}" / "page.md"
    for index, slug in (
...
def verify_judge_pages() -> None:
    ...
    logbook = json.loads(
        (VISIBLE_ROOT / "logbook.json").read_text(encoding="utf-8")
    )
    children = logbook["root"]["children"]
    current_files = [child["file"] for child in children[: len(JUDGE_PAGES)]]
    expected = [path.relative_to(VISIBLE_ROOT).as_posix() for path in JUDGE_PAGES]
    ...
    if not all(
        title.startswith("Historical rejected baseline")
...
def main() -> int:
    ...
    print(
        ...
        f"{evaluator_visibility['visibility_rows']}/6 claim rows complete; "
        f"{evaluator_visibility['first_hit_rows']} non-circular first-hit rows"
    )
